...
SELECT /*+ HIS.PA.AC.PI.SelectSeriousIllnessModificationApplicationFormCancelAsk */
 ...
 SELECT /* pkg_bil_srimd.pc_ap_apreghvt_resend_select2 */
       TO_CHAR(Z.DOC_NOTE_ID)                                                     AS DOC_NOTE_ID /* 진료기록ID */
     , TO_CHAR(Z.RLS_NO)                                                          AS RLS_NO      /* 개정번호 */
     , Z.PT_NO                                                                    AS PT_NO
     , Z.PT_NM                                                                    AS PT_NM       --성명
 ...
 where 1=1
   ...
   -- 2024-10-17 김용록 : 기간 -> 월 조회로 변경
   AND Z.CRT_DTE BETWEEN TO_DATe(:IN_REG_DT ||'01','YYYYMMDD') AND LAST_DAY(TO_DATe(:IN_REG_DT ||'01','YYYYMMDD'))
