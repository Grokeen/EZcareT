...
EXEC :FROMDATE := '20240930';
EXEC :TODATE := '20240930';
EXEC :IN_PT_NO := '01355858';
...
SELECT /*+ INDEX(ACPPRGHD APREGHVT_SI05) */ /* HIS.PA.AC.PI.PI.SELSERIOUSILLNESSAPPLICATIONFORMEDIREG1_1 */
       ...
       MIN(C16) AS C16,C17,C18,C19,C20,C21,C22,C23,C24,C25,C26,C27,
...
LEAST( NVL ( TO_CHAR ( A.CNCR_DGNS_CFMT_DT , 'YYYYMMDD' ) , TO_CHAR ( A.APFR_WRT_DT , 'YYYYMMDD' ) )
     , NVL ( TO_CHAR ( A.APFR_WRT_DT , 'YYYYMMDD' ) , '' ) ) C16,
...
DUP_CNCR_YN
   ...
   -- 2024-10-30 김용록 : 중복 제거(C16, 확진일을 최근 일자로 가져오게 처리)
   ) GROUP BY PT_NO,C1,C2,C3,C4,C5,C6,C7,C8,C9,C10,C11,C12,C13,C14,C15,--C16
