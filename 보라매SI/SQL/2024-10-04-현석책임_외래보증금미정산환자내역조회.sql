...
--            AND Z.HSP_TP_CD = A.HSP_TP_CD         -- 전남대 HSP_TP_CD 병원구분코드
            AND Z.CTAD_TP_CD = '00'
            AND ROWNUM = 1 ) TEL_NO



--     , TO_CHAR(nvl(A.OCUR_DT, A.RPY_DT),'yyyy-MM-dd') RPY_DT     -- 전남대 OCUR_DT 발생일자
     , A.RPY_AMT
     , A.RPY_STF_NO
--     , A.STRG_AMT_RMK                         -- 전남대 STRG_AMT_RMK 보관금액비고
     ...
     , ( SELECT TO_CHAR(MIN(X.MED_DT),'yyyy-MM-dd')                -- 전남대 MED_DT 진료일자
           FROM ACPPRODM X        -- 전남대 ACPPRODM 외래예약접수기본
          WHERE --X.HSP_TP_CD = :HIS_HSP_TP_CD
            -- AND
            X.MED_DT >= TO_DATE(:IN_TODATE ,'yyyymmdd')   --20220629 박수현 최근예약일을 당일포함하여 조회하는 것으로 변경(김미경선생님 요청)
            AND X.PT_NO = A.PT_NO
--            AND APCN_YN = 'N') RCN_RSV_DT       -- 전남대 APCN_YN 접수취소여부
...
--                    AND X.UNCL_AMT != X.UNCL_RPY_AMT) UNCL_AMT
 ...
 --           AND NVL(X.UNCL_RSN_CD,'*') &lt;&gt; '77'   -- 직원(급여공제)미수는 보이지 않도록 한다.(황계장)
  ...
  FROM ACPPEDPD A
     , PCTPCPAM B--, PCTPCPAM_DAMO B -- 전단대 환자기본

     --, PCTPCPTD C
 WHERE -- A.HSP_TP_CD = :HIS_HSP_TP_CD -- 전남대 HSP_TP_CD 병원구분코드
   --AND
