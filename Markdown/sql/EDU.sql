...
-- ROWCOUNT AND NOTFOUND PL/SQL 실습 
DECLARE
    CURSOR C_EMP_CURSOR IS 
        SELECT EMPLOYEE_ID, LAST_NAME
        FROM EMPLOYEES 
...
-- CREATE A PROCEDURE 실습
CREATE TABLE DEPT2 AS SELECT * FROM DEPARTMENTS;

CREATE PROCEDURE ADD_DEPT2 IS 
...
SET SERVEROUTPUT ON; -- DBMS_OUTPUT 메시지 버퍼의 출력이 표준 출력으로 경로 재지정되는지 여부를 지정
SELECT * FROM USER_SOURCE WHERE NAME = 'ADD_DEPT2';
