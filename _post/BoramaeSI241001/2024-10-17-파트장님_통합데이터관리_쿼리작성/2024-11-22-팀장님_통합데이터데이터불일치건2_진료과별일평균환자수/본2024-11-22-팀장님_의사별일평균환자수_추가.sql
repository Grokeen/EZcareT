...
SELECT
       CASE WHEN NVL(ASIS.WK_NM , TOBE.WK_NM) = '합계' THEN '합계'
            ELSE XBIL.FT_STF_INF(NVL(ASIS.WK_NM , TOBE.WK_NM ) , 'STF_NM')
        END                                       AS "지역별"
...
(
SELECT
       case grouping(WK_NM) when 0 then WK_NM
       when 1 then '합계'
       end                                              WK_NM --"지역별"
...
-- 이상우 : 환자번호, 진료과, 환자유형이 같지만 의사별로 카운트시 두건이 나올수 있음. 그래서 지역별, 진료과별과 다를수 있음.
-- ex. apoprsvt에 진료일자 2012-10-04, 환자번호 00283475 환자유형 BB

SELECT DECODE(WK_NM, NULL, '합계', WK_NM)         WK_NM
     , SUM(A)                                           A1-- "합계 보험"
     , SUM(B)                                           A2-- "합계 차상위"
     , SUM(C)                                           A3-- "합계 보호"
     , SUM(D)                                           A4-- "합계 일반"
...
ON ASIS.WK_NM = TOBE.WK_NM
 ORDER BY CASE WHEN NVL(ASIS.WK_NM , TOBE.WK_NM ) = '합계'
                    THEN 1
                    ELSE 0
           END
